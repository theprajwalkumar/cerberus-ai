...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.sync.set({
    hs_bridge_endpoint: DEFAULT_ENDPOINT,
  });
});

chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type !== "inject_hook") return;
  ...
  chrome.scripting.executeScript({
    ...
    world: "MAIN",
    func: injectHook,
    args: [msg.endpoint || DEFAULT_ENDPOINT, msg.sessionId],
  }).then(() => {
    sendResponse({ ok: true });
  }).catch((err) => {
    console.warn("[HS Bridge] executeScript error:", err.message);
...
function injectHook(endpoint, sessionId) {
  if (window.__hsBridgeInstalled) return;
  window.__hsBridgeInstalled = true;
  console.log("[HS Bridge] Live on (bg inject)", location.host);
